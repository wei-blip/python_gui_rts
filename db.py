...
class SqLiteDb:
    ...
    def put_data_into_db(self, data, comment_str):
        meas_id = (uuid.uuid1().int >> 68) % (2 ** 64)
        tm = datetime.datetime.now()
        if not self.sn_in_bd(data['uut']['sn']):
            self.__cursor.execute("INSERT INTO main(tm, sn, uut_dev_eui, uut_join_key, dev_type, last_fw_ver, "
                                  "hw_ver) VALUES (:tm, :sn, :uut_dev_eui, :uut_join_key, :dev_type, "
                                  ":last_fw_ver, :hw_ver)",
                                  {'tm':           tm,
                                   'sn':           data['uut']['sn'],
                                   ...
                                   'hw_ver':       1})
            self.__cursor.execute("INSERT INTO meas(tm,  main_sn, record_id, ref_dev_eui,  ref_sn,  fei,  snr,  per,  "
                                  "rssi,  pwr, ref_ds_t,  uut_ds_t,  ref_vcc,  uut_vcc,  ref_adc_t,  uut_adc_t,  "
                                  "ref_acc_x,  uut_acc_x,  ref_acc_y, uut_acc_y,  ref_acc_z,  uut_acc_z,  "
                                  "ref_vibro_x,  uut_vibro_x,  ref_vibro_y,  uut_vibro_y,  ref_vibro_z, uut_vibro_z,  "
                                  "fw_ver) VALUES (:tm, :main_sn, :record_id, :ref_dev_eui, :ref_sn, :fei, :snr, "
                                  ":per, :rssi, :pwr, :ref_ds_t, :uut_ds_t, :ref_vcc, :uut_vcc, :ref_adc_t, "
                                  ":uut_adc_t, :ref_acc_x, :uut_acc_x, :ref_acc_y, :uut_acc_y, :ref_acc_z, "
                                  ":uut_acc_z, :ref_vibro_x, :uut_vibro_x, :ref_vibro_y, :uut_vibro_y, :ref_vibro_z, "
                                  ":uut_vibro_z, :fw_ver)",
                                  {'tm':            tm,
                                   'main_sn':       data['uut']['sn'],
                                   'record_id':     meas_id,
        ...
        self.__cursor.execute("INSERT INTO comment(tm, main_sn, meas_id, comment_txt) VALUES "
                              "(:tm, :main_sn, :meas_id, :comment_txt)",
                              {'tm':            tm,
                               'main_sn':       data['uut']['sn'],
                               'meas_id':       meas_id,
                               'comment_txt':   comment_str})
        self.__connection.commit()
