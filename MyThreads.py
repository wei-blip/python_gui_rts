...
class ReaderSocketThread(QThread):
    ...
    def run(self):
        while self.win.socketIsConnect:
            try:
                data = self.win.messagesWindow.transport.recv(1024)
                if data:
                    self.buf.extend(data)
                    while self.win.messagesWindow.TERMINATOR in self.buf:
                        packet, self.buf = self.buf.split(self.win.messagesWindow.TERMINATOR, 1)
                        self.win.messagesWindow.handle_packet(packet)
